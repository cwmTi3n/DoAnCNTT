				...
				<table class="table table-hover">
					...
					<tbody>
						<c:forEach items="${phongs}" var="phong">
							<tr>
								<td class="col-2"><a
									href="<c:url value="/admin/phong?id_p=${phong.id_p }&id_taikhoan=${user.id_tk }"/>">
										<c:url value="/hinhanh?fname=${phong.anhchinh}" var="hinhanh" />
										<img height="130" width="180" src="${hinhanh}" />
								</a></td>
								<td class="col-2">${phong.ten }</td>
								<td class="col-1">${phong.gia}</td>
								<td class="col-1">${phong.chieudai}*${phong.chieurong}</td>
								<td class="col-2">${phong.getXa().getTenxa()}, ${phong.getXa().getHuyen().getTenhuyen()}, ${phong.getXa().getHuyen().getTinh().getTentinh() }</td>
								<td class="col-2">${phong.getTaikhoan().getTentk()}</td>
								<td class="col-2"><a
									href="<c:url value="/admin/xoa-phong?id_p=${phong.id_p }&id_taikhoan=${user.id_tk }"/>">Xóa</a></td>
